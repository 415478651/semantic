...
class convnextblock(layers.Layer):
    def __init__(self,
        ...
        self.conv11 = layers.Conv2D(
            ...
            use_bias=False,
            name=get_conv_name())
        self.norm11 = layers.LayerNormalization(
            axis=-1,
            epsilon=1e-3,
            name=get_norm_name())
        ...
        self.conv22 = layers.Conv2D(
            ...
            use_bias=False,
            name=get_conv_name())
        self.act22 = layers.Activation(tf.nn.relu)

        self.conv33 = layers.Conv2D(
            filters=out_c,
            kernel_size=1,
            strides=1,
            kernel_initializer=CONV_KERNEL_INITIALIZER,
    ...
    def call(self, inputs, training=None):
        ...
        x = self.conv11(x)
        x = self.norm11(x, training=training)
        x = self.conv22(x)
        x = self.act22(x)
        x = self.conv33(x)
...
def convnext_deeplabv3plus(num_classes: int = 1000,
                 name: str = None,
                 img_height = 512,
                 img_width = 512):
        ...
        img_input = layers.Input(shape=(img_width,img_height,3),dtype="float32")          
        x=conv(filters=96, kernel_size=4,strides=4,padding='same',kernel_initializer='he_normal', name='stem', use_bias=False)(img_input)
        x=convnextblock(input_c=96,out_c=96,expand_ratio=4,stride=1)(x)
        x=convnextblock(input_c=96,out_c=96,expand_ratio=4,stride=1)(x)
        x=convnextblock(input_c=96,out_c=192,expand_ratio=4,stride=1)(x)
       

        x=convnextblock(input_c=192,out_c=192,expand_ratio=4,stride=1)(x)
        x=convnextblock(input_c=192,out_c=192,expand_ratio=4,stride=1)(x)
        x=convnextblock(input_c=192,out_c=384,expand_ratio=4,stride=1)(x)
        x_b=0
        x_b=x

        x=convnextblock(input_c=384,out_c=384,expand_ratio=4,stride=2)(x)
        x=convnextblock(input_c=384,out_c=384,expand_ratio=4,stride=1)(x)
        x=convnextblock(input_c=384,out_c=384,expand_ratio=4,stride=1)(x)
        x=convnextblock(input_c=384,out_c=384,expand_ratio=4,stride=1)(x)
        x=convnextblock(input_c=384,out_c=384,expand_ratio=4,stride=1)(x)
        x=convnextblock(input_c=384,out_c=384,expand_ratio=4,stride=1)(x)
        x=convnextblock(input_c=384,out_c=384,expand_ratio=4,stride=1)(x)
        x=convnextblock(input_c=384,out_c=384,expand_ratio=4,stride=1)(x)
        x=convnextblock(input_c=384,out_c=768,expand_ratio=4,stride=1)(x)

        x=convnextblock(input_c=768,out_c=768,expand_ratio=4,stride=1)(x)
        x=convnextblock(input_c=768,out_c=768,expand_ratio=4,stride=1)(x)
        x=convnextblock(input_c=768,out_c=768,expand_ratio=4,stride=1)(x)
