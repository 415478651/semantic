...
#from model._model_transunet_2d import transunet_2d as create_model

from model.losses import dice
import os
ALPHA = 1.0
...
def getlines(path,labelpath):
    imgs = os.listdir(path)
    lines=[]
    for jpg in imgs:
        if jpg[-3:]=="tif" :
            line=path+'\\'+jpg+';'+labelpath+'\\'+jpg+'\n'
            #line=path+'\\'+jpg+';'+path[0:-3]+'label\\'+jpg+'\n'
            lines.append(line)
        #print(line)
...
def Diceloss(y_true,y_pred):
    ...
    return (1-f1)
@tf.function
def focalloss(ttensor,pretensor,alpha=0.75,gamma=2):
...
if __name__ == "__main__":
    log_dir = "logs/"
    # ��ȡmodel
    strategy = tf.distribute.MirroredStrategy(devices=["/gpu:0","/gpu:1","/gpu:2"],cross_device_ops=tf.distribute.HierarchicalCopyAllReduce())
    with strategy.scope():
    ...
    # ���ҵ����ݸ�������ѵ��
    '''
    lines0=getlines('E:\海漂垃圾自动识别\样本集\全部垃圾提取\惠安县山霞镇东埭村岸段img','E:\海漂垃圾自动识别\样本集\全部垃圾提取\惠安县山霞镇东埭村岸段label')
    lines1=getlines('E:\海漂垃圾自动识别\样本集\全部垃圾提取\蕉城区漳湾镇鳌江岸段img','E:\海漂垃圾自动识别\样本集\全部垃圾提取\蕉城区漳湾镇鳌江岸段label')
    lines2=getlines('E:\海漂垃圾自动识别\样本集\全部垃圾提取\霞浦溪南镇南坂至仙东岸段img','E:\海漂垃圾自动识别\样本集\全部垃圾提取\霞浦溪南镇南坂至仙东岸段label')
    lines3=getlines('E:\海漂垃圾自动识别\样本集\全部垃圾提取\霞浦县下浒镇前洋至西岐岸段img','E:\海漂垃圾自动识别\样本集\全部垃圾提取\霞浦县下浒镇前洋至西岐岸段label')
    lines4=getlines('E:\海漂垃圾自动识别\样本集\全部垃圾提取\秀屿区东埔镇塔林岸段img','E:\海漂垃圾自动识别\样本集\全部垃圾提取\秀屿区东埔镇塔林岸段label')
    lines=lines0+lines1+lines2+lines3+lines4'''

    
    lines=getlines("E:\\海漂垃圾自动识别\\样本集\\废弃撑架\\positive\\img256","E:\\海漂垃圾自动识别\\样本集\\废弃撑架\\positive\\label256")
    #lines2=getlines(r"E:\海漂垃圾自动识别\样本集\废弃撑架\negtive\img",r"E:\海漂垃圾自动识别\样本集\废弃撑架\negtive\label")
    #np.random.shuffle(lines2)
    #lines=lines1+lines2[0:9000]
    '''
    
    lines1=getlines(r"Y:\wsw\wsw2\海漂垃圾\泡沫浮球样本库\蕉城区漳湾镇拱屿岸段img",r"Y:\wsw\wsw2\海漂垃圾\泡沫浮球样本库\蕉城区漳湾镇拱屿岸段label")
    lines2=getlines(r"Y:\wsw\wsw2\海漂垃圾\泡沫浮球样本库\蕉城区漳湾镇漳湾岸段img",r"Y:\wsw\wsw2\海漂垃圾\泡沫浮球样本库\蕉城区漳湾镇漳湾岸段label")
    lines3=getlines(r"Y:\wsw\wsw2\海漂垃圾\泡沫浮球样本库\罗源县碧里乡廪尾岸段img",r"Y:\wsw\wsw2\海漂垃圾\泡沫浮球样本库\罗源县碧里乡廪尾岸段label")
    lines4=getlines(r"Y:\wsw\wsw2\海漂垃圾\泡沫浮球样本库\连江县马鼻镇合丰岸段img",r"Y:\wsw\wsw2\海漂垃圾\泡沫浮球样本库\连江县马鼻镇合丰岸段label")
    lines5=getlines(r"Y:\wsw\wsw2\海漂垃圾\泡沫浮球样本库\平潭苏澳镇苏澳至斗魁岸段img",r"Y:\wsw\wsw2\海漂垃圾\泡沫浮球样本库\平潭苏澳镇苏澳至斗魁岸段label")
    
    lines=lines1+lines2+lines3+lines4+lines5
    '''
    #lines=getlines(r"E:\海漂垃圾自动识别\样本集\全部垃圾提取\positive\img",r"E:\海漂垃圾自动识别\样本集\全部垃圾提取\positive\label")
    #print(lines)
